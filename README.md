...
Madoiは、メッセージ配信機能とルーム管理機能、オブジェクト管理機能から構成され、JavaScriptもしくはTypeScriptで開発されるWebアプリケーションから利用することを想定しています。メッセージ配信機能およびルーム管理機能だけでメッセージ配信サーバとして利用可能ですが、オブジェクト管理機能を利用すると、オブジェクトの状態同期に必要な処理の多くをMadoiに任せることができ、より効率的にコラボレーション機能を開発できます。
...
* クライアントアプリ(JavaScript/TypeScript)
  * 共有オブジェクト
    * アプリ毎に開発する、他のアプリと状態を共有するオブジェクト。チャットの発言リストや描画中の画像など、内部で保持する状態の変更や取得・設定を行います。
  * Madoiクライアント
    * Madoiサーバと接続し、共有オブジェクトの状態の同期や各種通信関連イベントの通知を行います。
* Madoiサーバ
  * 共有オブジェクト管理
...
Madoiサーバでは、共有オブジェクトの管理を行いますが、共有オブジェクトの実装には関与しません。あくまで共有オブジェクトのメタデータ(クラス名や共有対象のメソッド名等)、共有オブジェクトから取得した状態(JSON形式)や変更メソッドの呼び出し(メソッド名+パラメータをJSON形式に変換したもの)を扱い、それらの管理と他の参加者への配信を行います。クライアントアプリの実装に依存しない、汎用的なサーバ機能として実装されています。
...
`Started Application`と出力されると、サーバが起動しています。この状態で `http://localhost:8080/madoi/chat_by_sendrecv.html` にブラウザでアクセスすると、チャットサンプルが表示されます。
...
- 中口孝雄, "分散情報共有サービスMadoiを用いたコラボレーションツールの宣言的プログラミング ," 電子情報通信学会技術研究報告, 2025.8.
